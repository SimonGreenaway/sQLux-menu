...
public class SqluxMenu extends javax.swing.JFrame
{
    ...
    public void fromIni(final Ini ini)
    {
        ...
        if(ini.has("SQLUXPATH")) sqluxPathTextField.setText(ini.get("SQLUXPATH").stream().findAny().get());
    ...
    public void toIni(final Ini ini)
    {
        ...
        if(!romdir.isBlank()&&(sysrom.length()>romdir.length())&&sysrom.startsWith(romdir)) sysrom=sysrom.substring(romdir.length()+1);
